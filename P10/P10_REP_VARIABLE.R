...
efi_1_REP=(optimo_1 - mejor_1) / optimo_1
efi_2_REP=(optimo_2 - mejor_2) / optimo_2
efi_3_REP=(optimo_3 - mejor_3) / optimo_3

REP=as.character(reproducciones)
treatment=rep(REP,each=replicas)
generacion=as.character(rep(1:3,each=replicas*length(REP)))
eficiencias=c(efi_1_REP,efi_2_REP,efi_3_REP)
data_REP=data.frame(eficiencias, treatment,generacion)
library(ggplot2)
ggplot(data_REP, aes(x=generacion, y=eficiencias, fill=treatment)) +
geom_boxplot() +ylim(0.01, 0.3)+
  xlab("Método de generación") + ylab("Relación mejor/óptimo")+labs(fill = "Impacto del número de reproducciones")
