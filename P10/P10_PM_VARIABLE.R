...
efi_1_PM=(optimo_1 - mejor_1) / optimo_1
efi_2_PM=(optimo_2 - mejor_2) / optimo_2
efi_3_PM=(optimo_3 - mejor_3) / optimo_3

PM=as.character(probmut)
treatment=rep(PM,each=replicas)
generacion=as.character(rep(1:3,each=replicas*length(probmut)))
eficiencias=c(efi_1_PM,efi_2_PM,efi_3_PM)
data_PM=data.frame(eficiencias, treatment,generacion)
library(ggplot2)
ggplot(data_PM, aes(x=generacion, y=eficiencias, fill=treatment)) +
geom_boxplot() +ylim(0.01, 0.3)+
  xlab("Método de generación") + ylab("Relación mejor/óptimo")+labs(fill = "Impacto de la probabilidad de mutación")